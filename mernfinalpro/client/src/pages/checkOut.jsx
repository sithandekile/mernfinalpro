...
export const Checkout = () => {
  ...
  if (deliveryOption === 'delivery' && (!formData.address || !formData.city)) {
    setMessage("Please fill in address and city for delivery.");
    return;
  }
  ...
  try {
    ...
    if (!orderResponse.success) {
      setMessage(orderResponse.error || 'Failed to create order');
      setProcessing(false);
      return;
    }

    // Step 2: Init payment
    const totalAmount = Math.round(total * 100); // convert to smallest currency unit

    const paymentResponse = await apiService.initPayment({
      email: formData.email,
      amount: totalAmount,
      currency: 'ZAR',
      orderId: orderResponse.data._id // now it's defined
    });
    ...
    if (paymentResponse.success && paymentResponse.paymentUrl) {
      ...
      setMessage(paymentResponse.error || 'Failed to initialize payment');
    }
  } catch (error) {
          ...
          <div>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button type="submit" disabled={processing}
                className="w-full bg-orange-600 hover:bg-orange-700 disabled:bg-gray-400 text-white px-6 py-4 rounded-xl text-lg font-semibold">
                {processing ? 'Processing...' : `Pay with Paystack - $${total}`}
              </button>
            </form>
